...
export async function getStaticProps({ params }) {
  const urls = dataUrls();
  const itemData = await getItemData(params.id, urls.serviceDataUrl);
  return {
    props: {
      itemData,
    },
...
export async function getStaticPaths() {
  const urls = dataUrls();
  const paths = await getIds(urls.serviceDataUrl);
  return {
    paths,
    fallback: false,
  };
        ...
        >
          ...
          <Box>
            ...
            <Text
              ...
              fontWeight={"300"}
            >
              {itemData.acf_fields.prerequisites}
            </Text>
          </Box>
        </Stack>
        <Center my={10}>
